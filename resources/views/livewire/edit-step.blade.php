<div>
    ...
    @foreach( $steps as $step )
        <div class="flex justify-center">
            ...
            <input type="text" name="stepName[]" class="px-2 py-1 m-1 border rounded" placeholder="{{'Describe step '.($loop->index + 1)}}"   wire:model="steps.{{$loop->index}}.{{'name'}}"/>

            <input type="text" name="stepId[]" class="hidden" value="{{$step['id']}}"/>

            <!-- <input type="text" name="stepId[]" class="hidden" wire:model="steps.{{$loop->index}}.{{'id'}}"/> -->
